  ...
  return (
    <>
      <div
        ...
        } h-64 w-[25rem] rounded-xl flex flex-col justify-start p-2 items-center shadow-2xl `}
      >
        <p>5 Days Forecast:</p>
        ...
        <div
          className={`${
            loading
              ...
              : ""
          } h-8 flex justify-around items-center`}
        >
          {loading ? (
            ""
          ) : (
            <>
              <i
                className={` w-10 flex justify-center items-center ${icons.map(
                  (icon) => icon[data[value].icon]
                )} text-2xl`}
              ></i>
              <span className=" w-14 flex justify-center items-center">
                {!activeC
                  ? `${Math.ceil(
                      CelciusToFahrenheit(parseInt(data[value].temp))
                    )}°C`
                  : `${data[value].temp}°F`}
              </span>
              <span className=" w-28 flex justify-center items-center">{`${
                days[(date.getDay() + value) % 7]
              },${date.getDate() + value} ${
